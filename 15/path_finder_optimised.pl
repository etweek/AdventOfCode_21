...
for(my $i=0; $i<$numrows; $i++){
  for(my $j=0; $j<$numcols; $j++){
    ...
    $unvisited->{$key} = 10*$numrows*$numcols;
    $cost->{$key} = $map[$i][$j];
  }
}
...
sub calculate_node{
  ...
  for(my $n_i=($i>0?$i-1:$i);   $n_i <= ($i==($numrows-1)?$i:$i+1); $n_i++){
    for(my $n_j=($j>0?$j-1:$j); $n_j <= ($j==($numcols-1)?$j:$j+1); $n_j++){
      ...
      if(defined($visited->{$n_key})){
        next;
      }
          
      #print "\tChecking $n_i,$n_j: ";
      if(!defined($partial->{$n_key})){
        # bring it over from the unvisited set
        $partial->{$n_key} = $unvisited->{$n_key};
        delete($unvisited->{$n_key});
      }
      my $current = $partial->{$n_key};
      if($current > $path_cost+$cost->{$n_key}){
        # shorter path found
        $partial->{$n_key} = $path_cost+$cost->{$n_key};
      }
...
sub cheapest_node{
  ...
  if(scalar(%$partial) > 0){
    my @keys = sort { $partial->{$a} <=> $partial->{$b} } keys(%$partial);
    return $keys[0];
  }
  elsif(scalar(%$unvisited)>0){
    return @{keys(%$unvisited)}[0];
  }
  else{
    die "Ran out of elements\n";
  }
}
