...
my $first_scanner;
my $beacons;
for (my $i=0; $i < $#scannernames; $i++){
  $total = $total + scalar(@{$scanners->{$scannernames[$i]}->{"beacons"}});
  for (my $j=$i+1; $j <= $#scannernames; $j++){
    print "\t\tConsidering ",$scannernames[$i],"<->",$scannernames[$j],"\n";

    #$total = $total + scalar(@{$scanners->{$scannernames[$j]}->{"beacons"}});
    my $common = compute_union($scanners->{$scannernames[$i]}, $scanners->{$scannernames[$j]});
    print $scannernames[$i],"<->",$scannernames[$j],": ",$common->{"count"},"\n";
    if($common->{"count"} >= 12){
      if(!defined($first_scanner)){
        #First!!!
        print Dumper($common->{"matrix"});
        $first_scanner = $scannernames[$i];
        for my $beacon (@{$scanners->{$scannernames[$i]}->{"beacons"}}){
          $beacons->{format_beacon($beacon)} = 1;
        }
        for my $beacon (@{$scanners->{$scannernames[$j]}->{"beacons"}}){
          $beacon = scanner_map($beacon, $common->{"matrix"});
          $beacons->{format_beacon($beacon)}++;
        }
        $matrices->{$scannernames[$j]}->{$first_scanner}=$common->{"matrix"};
      }
      else{
        if($scannernames[$i] eq $first_scanner){
          # phew, no double-mapping
          print Dumper($common->{"matrix"});
          for my $beacon (@{$scanners->{$scannernames[$j]}->{"beacons"}}){
            $beacon = scanner_map($beacon, $common->{"matrix"});
            $beacons->{format_beacon($beacon)}++;
          ...
          if(defined($matrices->{$scannernames[$i]}->{$first_scanner})){
            #double map
            print Dumper($common->{"matrix"});
            my $intermediate = $matrices->{$scannernames[$i]}->{$first_scanner};
            print Dumper($intermediate);
            for my $beacon (@{$scanners->{$scannernames[$j]}->{"beacons"}}){
#              $beacon = scanner_map(scanner_map($beacon, $common->{"matrix"}), $intermediate);
              $beacon = scanner_map($beacon, $common->{"matrix"});
              $beacons->{format_beacon($beacon)}++;
            }
          }
...
$total = $total + scalar(@{$scanners->{$scannernames[-1]}->{"beacons"}});
