...
sub manhattan{
  my $scanner1 = shift;
  my $scanner2 = shift;
  return abs($scanner1->{"x_offset"}-$scanner2->{"x_offset"}) +
      abs($scanner1->{"y_offset"}-$scanner2->{"y_offset"}) +
      abs($scanner1->{"z_offset"}-$scanner2->{"z_offset"});
}

my $scanner_maps;
#include the first one just in case
my $origin;
$origin->{"x_offset"}=0;
$origin->{"y_offset"}=0;
$origin->{"z_offset"}=0;
$scanner_maps->{$first_scanner} = $origin;
for my $scanner (keys(%$matrices)){
  $scanner_maps->{$scanner} = $matrices->{$scanner}->{$first_scanner};
}
my $largest = 0;
my $largest_pair="";
for my $scanner1 (keys(%$scanner_maps)){
  for my $scanner2 (keys(%$scanner_maps)){
    next if $scanner1 eq $scanner2;
    my $distance = manhattan($scanner_maps->{$scanner1}, $scanner_maps->{$scanner2});
    if($distance > $largest){
      $largest = $distance;
      $largest_pair=$scanner1."<->".$scanner2;
    }
  }
}
print "Largest distance, between $largest_pair was $largest\n";
    
#print Dumper($scanner_maps);
